...
public static function changeState($diff)
{
    $unmodified = true; 
    $onlyInsert = true;
    $index = 0;
   ...
   *                defaults to '<br>'
   */
  public static function toTable($diff, $indentation = '', $separator = '<br>',$head = null){
    ...
    if($head != null)
    {
        ...
        foreach($head as $field)
        {
            if(array_key_exists('class', $field) )
                $html .= '<td class="'.$field['class'].'">'.$field['name'].'</td>';
            else
    ...
    while ($index < count($diff)){
      ...
      $html .=
          ...
          . "  <tr>\n"
          . $indentation
          . '<td>'.($leftCell == $rightCell?'':($leftCell == '' ? '' : '-')).'</td>'
          . '    <td class="diff'
          . ($leftCell == $rightCell
              ? 'Unmodified'
          ...
          . "</td>\n"
          . $indentation
          . '<td>'.($leftCell == $rightCell?'':($rightCell == '' ? '' : '+')).'</td>'
          . '    <td class="diff'
          . ($leftCell == $rightCell
              ? 'Unmodified'
